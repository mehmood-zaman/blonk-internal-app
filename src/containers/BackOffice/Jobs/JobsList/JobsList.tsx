...
const JobsList: React.FC = () => {
  ...
  useEffect(() => {
    setFakeData({
      ...fakeData,
      jobRecorda: [...generateJobs(20, url).jobs],
    });
    setTimeout(function () {
      setLoading(false);
    }, 1000);
  ...
  const columns: ColumnsType<Jobs> = [
    {
      title: 'Job Title',
      dataIndex: 'link',
      key: 'link',
    ...
    },
    {
      ...
      render: (location) => (
        <>
          {location.map((el: any) => {
            return <Tag key={el}>{el.toUpperCase()}</Tag>;
          })}
        </>
      ),
    },
    {
      ...
      dataIndex: 'id',
      width: 100,
      render: (id, row) => candidatesHanlder(id, row.Candidates),
    ...
    },
    {
      ...
      dataIndex: 'id',
      width: 100,
      render: (id, row) => matchesHanlder(id, row.Candidates),
                ...
                <Tabs defaultActiveKey="1">
                  ...
                  >
                    <Table<Jobs>
                      ...
                      columns={columns}
                      dataSource={fakeData.jobRecorda}
                      loading={loading}
                      size="small"
                  ...
                  <TabPane
                    tab={
                      <span>
                        <i className="dripicons-archive " /> Archived jobs
                        <span className="tab-pane-count">02</span>
                      </span>
                    }
                    key="2"
                  >
                    <Table<Jobs>
                      columns={columns}
                      bordered={true}
                      dataSource={generateJobs(2, url).jobs}
                      size="small"
                      pagination={{ pageSize: 10 }}
                      scroll={{ x: 1300 }}
                      className="table-striped"
                  ...
                  >
                    <Table<Jobs>
                      columns={columns}
                      bordered={true}
                      dataSource={generateJobs(12, url).jobs}
                      size="small"
                      pagination={{ pageSize: 10 }}
                      scroll={{ x: 1300 }}
                      className="table-striped"
                  ...
                  >
                    <Table<Jobs>
                      columns={columns}
                      bordered={true}
                      dataSource={generateJobs(8, url).jobs}
                      size="small"
                      pagination={{ pageSize: 10 }}
                      scroll={{ x: 1300 }}
                      className="table-striped"
